...
FILES=(
  ...
  "templates/emails/generic.html.twig"
  "templates/emails/generic.txt.twig"
  "src/Admin/Presentation/EasyAdmin/BillingPlanCrudController.php"
  "src/Identity/Application/Service/IdentityManager.php"
  "src/Shared/Presentation/Controller/HealthController.php"
  "src/Shared/Infrastructure/Outbox/DispatchOutboxMessageHandler.php"
  "src/Shared/Infrastructure/Outbox/OutboxPayloadProtector.php"
  "tests/Functional/AdminAccessTest.php"
...
for file in "${FILES[@]}"; do
  ...
  replace_literal "$target" "my-app/back" "${PROJECT_NAME}/back"
  ...
  replace_literal "$target" "My App Back API" "${PROJECT_TITLE} Back API"
  ...
  replace_literal "$target" "My App back" "${PROJECT_TITLE} back"
  replace_literal "$target" "My App Admin" "${PROJECT_TITLE} Admin"
  replace_literal "$target" "Starter backend Symfony DDD centré sur \`User\`." "${PROJECT_TITLE} Back API Symfony DDD centree sur \`User\`."
  ...
  replace_literal "$target" "X-My-App-Outbox-Id" "X-${PROJECT_HEADER_PREFIX}-Outbox-Id"
  replace_literal "$target" "/run/starter-secrets/jwt" "$SECRETS_DIR"
  replace_literal "$target" "/run/my_app-secrets/jwt" "$SECRETS_DIR"
  replace_literal "$target" "My App" "$PROJECT_TITLE"
  replace_literal "$target" "MY_APP" "$PROJECT_UPPER"
  replace_literal "$target" "my_app" "$PROJECT_DB_IDENTIFIER"
  replace_literal "$target" "starter_monthly" "${PROJECT_NAME}_monthly"
done
...
replace_literal "${ROOT_DIR}/.env.example" "mysql://my_app:my_app@db:3306/my_app?serverVersion=8.0.32&charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@db:3306/${PROJECT_DB_IDENTIFIER}?serverVersion=8.0.32&charset=utf8mb4"
if [ -f "${ROOT_DIR}/.env" ]; then
  replace_literal "${ROOT_DIR}/.env" "mysql://app:app@db:3306/starter?serverVersion=8.0.32&charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@db:3306/${PROJECT_DB_IDENTIFIER}?serverVersion=8.0.32&charset=utf8mb4"
  replace_literal "${ROOT_DIR}/.env" "mysql://my_app:my_app@db:3306/my_app?serverVersion=8.0.32&charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@db:3306/${PROJECT_DB_IDENTIFIER}?serverVersion=8.0.32&charset=utf8mb4"
  replace_literal "${ROOT_DIR}/.env" "/run/starter-secrets/jwt" "$SECRETS_DIR"
  replace_literal "${ROOT_DIR}/.env" "/run/my_app-secrets/jwt" "$SECRETS_DIR"
  replace_literal "${ROOT_DIR}/.env" "My App" "$PROJECT_TITLE"
  replace_literal "${ROOT_DIR}/.env" "my_app" "$PROJECT_DB_IDENTIFIER"
fi
...
replace_literal "${ROOT_DIR}/.env.test" "mysql://my_app:my_app@db:3306/my_app_test?serverVersion=8.0.32&charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@db:3306/${PROJECT_DB_IDENTIFIER}_test?serverVersion=8.0.32&charset=utf8mb4"
replace_literal "${ROOT_DIR}/docker-compose.yml" "MYSQL_DATABASE: starter" "MYSQL_DATABASE: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.yml" "MYSQL_DATABASE: my_app" "MYSQL_DATABASE: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.yml" "MYSQL_USER: app" "MYSQL_USER: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.yml" "MYSQL_USER: my_app" "MYSQL_USER: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.yml" "MYSQL_PASSWORD: app" "MYSQL_PASSWORD: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.yml" "MYSQL_PASSWORD: my_app" "MYSQL_PASSWORD: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.test.yml" "MYSQL_DATABASE: starter_test" "MYSQL_DATABASE: ${PROJECT_DB_IDENTIFIER}_test"
replace_literal "${ROOT_DIR}/docker-compose.test.yml" "MYSQL_DATABASE: my_app_test" "MYSQL_DATABASE: ${PROJECT_DB_IDENTIFIER}_test"
replace_literal "${ROOT_DIR}/docker-compose.test.yml" "MYSQL_USER: app" "MYSQL_USER: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.test.yml" "MYSQL_USER: my_app" "MYSQL_USER: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.test.yml" "MYSQL_PASSWORD: app" "MYSQL_PASSWORD: ${PROJECT_DB_IDENTIFIER}"
replace_literal "${ROOT_DIR}/docker-compose.test.yml" "MYSQL_PASSWORD: my_app" "MYSQL_PASSWORD: ${PROJECT_DB_IDENTIFIER}"
for workflow_file in "${ROOT_DIR}/.github/workflows/ci.yml" "${ROOT_DIR}/.github/workflows/build-and-push.yml"; do
  replace_literal "$workflow_file" "-e MYSQL_DATABASE=starter" "-e MYSQL_DATABASE=${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "-e MYSQL_DATABASE=my_app" "-e MYSQL_DATABASE=${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "-e MYSQL_USER=app" "-e MYSQL_USER=${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "-e MYSQL_USER=my_app" "-e MYSQL_USER=${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "-e MYSQL_PASSWORD=app" "-e MYSQL_PASSWORD=${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "-e MYSQL_PASSWORD=my_app" "-e MYSQL_PASSWORD=${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "-uapp -papp" "-u${PROJECT_DB_IDENTIFIER} -p${PROJECT_DB_IDENTIFIER}"
  replace_literal "$workflow_file" "mysql://app:app@\${db_name}:3306/starter?serverVersion=8.0.32&charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@\${db_name}:3306/${PROJECT_DB_IDENTIFIER}?serverVersion=8.0.32&charset=utf8mb4"
  replace_literal "$workflow_file" "mysql://my_app:my_app@\${db_name}:3306/my_app?serverVersion=8.0.32&charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@\${db_name}:3306/${PROJECT_DB_IDENTIFIER}?serverVersion=8.0.32&charset=utf8mb4"
done
...
replace_literal "${ROOT_DIR}/phpunit.dist.xml" "mysql://my_app:my_app@db:3306/my_app_test?serverVersion=8.0.32&amp;charset=utf8mb4" "mysql://${PROJECT_DB_IDENTIFIER}:${PROJECT_DB_IDENTIFIER}@db:3306/${PROJECT_DB_IDENTIFIER}_test?serverVersion=8.0.32&amp;charset=utf8mb4"
